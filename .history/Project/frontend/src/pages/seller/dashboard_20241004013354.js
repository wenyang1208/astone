...
function Dashboard() {
    ...
    const TodoCard = ({ value, label, showDivider = true, onClick }) => (
        <StatsContainer>
            ...
            {/* If onClick is provided, render label as a Button */}
            {onClick ? (
                <Button variant="text" sx={{ color: '#666', fontSize: '0.95rem' }} onClick={onClick}>
                    {label}
                </Button>
            ) : (
            ...
            {items.map((item, index) => (
                <Grid item xs={3} key={item.label}>
                    <TodoCard 
                        ...
                        onClick={item.onClick}  // Pass onClick handler only for clickable items
                    />
                </Grid>
            ))}
        </Grid>
    ...
    const firstRowItems = [
        ...
        { 
            value: todoData.toProcessShipment, 
            label: "To-Process Shipment", 
            onClick: () => navigate('/orders/to-process')  // Make this item clickable
        },
